  ...
  btn.addEventListener("click", async () => {
    ...
    if(!r.ok){
      const reasonMsg = {
        ...
        "already":  t("redeem.err.already")  || "Ви вже активували цей код",
        "offline":  t("redeem.err.offline")  || "Немає інтернету для перевірки коду"
...
function paintShopTabs(){
  ...
  /* Make sure the active tab is reachable on narrow phones where the
     strip overflows horizontally. scrollIntoView with inline:"center"
     leans on the browser's native smooth scroller (we set
     scroll-behavior:smooth in CSS), so it stays jank-free. */
